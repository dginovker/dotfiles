...
13. Per-tool breakdown in Tool results (API-verifiable): Send a message triggering 3+ different tools (e.g., "Get the scene tree, list errors, and get project info"), call `/get-context-usage`, assert `toolCallDetails` is an array with length >= 3, each entry has `name` (string), `tokens` (number > 0), `count` (number > 0). Assert `sum(toolCallDetails[*].tokens)` equals `currentBreakdown.toolResults` within ±1
14. Context files vs System prompt split (API-verifiable): Send a message, call `/debug-token-breakdown`, verify breakdown has separate `contextFiles` and `systemPrompt` fields (both > 0 when files are open). Verify `contextFileDetails` array has entries with per-file token counts. Verify `contextFileDetailsSum` equals `contextFiles` exactly (`contextFilesMatch: true`)
15. Token breakdown accuracy (API-verifiable): Send a message triggering 10+ tool calls, call `/debug-token-breakdown`, verify `contextFiles + systemPrompt + tools + toolResults + messages` equals `breakdownSum` exactly. Verify cumulative equivalent matches too. No "Other" line in widget
16. Context files parent-child consistency (API-verifiable): Send 2+ messages with tool calls, call `/get-context-drift`, assert `driftPct == 0`. Both parent and children come from the server — zero drift by construction
17. Reasoning token tracking (API-verifiable): Switch to a model with reasoning support (e.g., Gemini via `/set-model` with `{"modelId": "google/gemini-2.0-flash-exp"}`), send a message requiring thinking (e.g., "Add a jump mechanic to the player"), call `/get-context-usage`, assert `reasoningTokens > 0` and `cumulativeReasoningTokens > 0`. Verify reasoning tokens are tracked separately from output tokens
18. Reasoning token display in widget (screenshot): With reasoning tokens tracked from test #17, open context usage dialog, expand "Output from LLM", verify two sub-items appear: "Generated text" (with token count and cost) and "Reasoning" (with token count and cost). Verify "Generated text" + "Reasoning" equals total output tokens. Switch to "Cumulative Cost" view and verify cumulative reasoning tokens are displayed
