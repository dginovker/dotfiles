...
14. Context files vs System prompt split (API-verifiable): Send a message, call `/get-context-usage`, verify the widget shows "Context files" and "System prompt" as separate rows under "Input to LLM". Their displayed values should sum to `currentBreakdown.system`. No sub-item should exceed the total "Input to LLM" value
15. Tool output counting accuracy (API-verifiable): Send a message triggering 10+ tool calls, call `/get-context-usage`, calculate Other = `cumulativeBreakdown.system + cumulativeBreakdown.tools + cumulativeBreakdown.toolResults + cumulativeBreakdown.messages` - verify this sum equals `cumulativeInputTokens` exactly (Other = 0 by construction). Alternatively verify no "Other" line appears in the widget. Pass threshold: other < 50000
16. Context files parent-child consistency (API-verifiable): Send 2+ messages with tool calls, call `/get-context-drift`, assert `driftPct == 0` (children are proportionally derived from parent). This validates that agents.md + open scripts + open scenes sum exactly to the Context files total
